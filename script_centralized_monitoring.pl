...
sub format_uptime {
    ...
    # Parse ISO 8601 timestamp (e.g., 2024-05-20T10:00:00)
...
# Using ->> operator (JSON_UNQUOTE) to ensure the port is returned as a raw string
my $sql = q{
SELECT
  router_id,
  address,
...
while (my $row = <$mysql_fh>) {
    ...
    # Querying the Router's status endpoint
    ...
    if ($response) {
        ...
        if ($data && ref($data) eq 'HASH') {
            ...
            # If we got a valid version and hostname, consider the router UP
            $status   = ($data->{version} && $hostname ne 'unknown') ? 1 : 0;
        }
    }

    # Output formatted for OEM Metric Extension (Multi-column result)
    print join('|', $router_id, $address, $port, $status, $hostname, $uptime), "\n";
}
